...
!========================================================================
module htm_v3_network
    use, intrinsic :: ISO_FORTRAN_ENV, only: INT8, INT16, INT32, INT64, REAL32, REAL64
    use tools
    use htm_v3_constants
    use htm_v3_tools
    use htm_v3_encoder, only: encode_pass_through, fetch_sensor_activity, print_sensor_activity2, print_inferred_sensor_activity, encoder_cleanup
    use htm_v3_sp,      only: compute_sp
    use htm_v3_tm,      only: compute_tm
    use htm_v3_tp,      only: compute_tp
    use htm_v3_tp2,     only: compute_tp2
    use htm_v3_print
...
!========================================================================
    subroutine run(n_time_steps, progress)
        ...
        do t = 1,n_time_steps
            ...
            call add_sensor_noise(sensor_activity)
...
#           endif
            ...
            if (.TRUE.) then
                ...
                if (MOD(t, 100) == 0) then
                    write(*, '("INFO:run: t"i6": total mismatch "i6"; average mismatch ="f)', advance='yes'), t, total_mismatch, REAL(total_mismatch) / (N_SENSORS * 100)
                    total_mismatch = 0
                end if
            end if
...
!========================================================================
    subroutine add_sensor_noise(sensor_activity)
        logical(1),     intent(inout) :: sensor_activity(N_SENSORS)
        ! local variables
        integer(INT32), parameter :: RAND_PERCENT = 1
        integer(INT32) :: i
        
        i = rand_int32(1, RAND_PERCENT*200)
        do while (i <= N_SENSORS) 
            sensor_activity(i) = NOT(sensor_activity(i))
            i = i + rand_int32(1, RAND_PERCENT*200)
        end do
    end subroutine
!========================================================================
    subroutine init(param)
        ...
        do column_i = 1,param % n_columns
            ...
            do synapse_i = 1,N_PD_SYNAPSES
                ...
                columns(column_i) % pd_synapse_origin(synapse_i) = rand_int32(1, N_SENSORS)
            end do
