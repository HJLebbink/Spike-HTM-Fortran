        ...
        if (.FALSE.) then
            ...
            do synapse_i = 1, N_PD_SYNAPSES
                if (is_PD_connected(column, synapse_i, SP_PD_PERMANENCE_THRESHOLD)) then
                    sensor_idx = columns(column) % proximal_dendrite_synapse_origin(synapse_i)
            ...
            block
                ...
                x = (columns(column) % proximal_dendrite_synapse_permanence >= SP_PD_PERMANENCE_THRESHOLD).AND.sensor_activity(columns(column) % proximal_dendrite_synapse_origin)
                overlap = SUM(x)
...
#               endif
                ...
                if (learn) then
                    do synapse_i = 1,N_PD_SYNAPSES
                        sensor_idx = columns(column) % proximal_dendrite_synapse_origin(synapse_i)
                        old_permanence = columns(column) % proximal_dendrite_synapse_permanence(synapse_i)

                        !print '("INFO:SP:inhibition:      old permanance for synpase ",i5," in column ",i4,":",f,").")', synapse_i, column_i, columns(column_i) % proximal_dendrite_synapse_permanance(synapse_i)
                        if (sensor_activity(sensor_idx)) then
...
#                       endif
                        columns(column) % proximal_dendrite_synapse_permanence(synapse_i) = new_permanence
                    end do
                end if
            end if
        end if
...
!========================================================================
    elemental function is_PD_connected(column, synapse, permanence_threshold) result(r)
        ...
        logical(1) :: r
        r = (columns(column) % proximal_dendrite_synapse_permanence(synapse) >= permanence_threshold)
...
#           endif
            ...
            do synapse_i = 1, N_PD_SYNAPSES
                if (NOT(is_PD_connected(column, synapse_i, SP_PD_PERMANENCE_THRESHOLD))) then
                    if (columns(column) % proximal_dendrite_synapse_origin(synapse_i) /= -1) then
                        columns(column) % proximal_dendrite_synapse_permanence(synapse_i) = &
                            columns(column) % proximal_dendrite_synapse_permanence(synapse_i) + PD_PERMANENCE_INC_WEAK
                    end if
                end if
            end do
        end if
