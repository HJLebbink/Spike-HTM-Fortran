...
!========================================================================
    subroutine print_bitset(bitset)
        ...
        integer(INT32) :: i
        write(*, '("(count ="(1X,I0)"):")', advance='no'), count_active_logical(bitset)
        do i = 1,SIZE(bitset)
            if (bitset(i)) then
...
!========================================================================
    subroutine print_active_cells(bitset)
        ...
        do column_i = 1,N_COLUMNS
            active_column(column_i) = is_not_empty_64(bitset(column_i))
            if (active_column(column_i)) then
                n_active_columns = n_active_columns + 1
            end if
        end do
        n_active_cells = count_active_64_array(bitset)
        ...
        if (n_active_columns > 0) then
            ...
            do cell_i = 1,N_CELLS_PC
                do column_i = 1,N_COLUMNS
                    if (active_column(column_i)) then
                        if (get_element_64(bitset(column_i), cell_i)) then
                            write(*, '("    X")', advance='no')
                        else
                            write(*, '("    .")', advance='no')
                        end if
...
!========================================================================
    subroutine print_segment_activity(                                  &
        ...
        ! local variables
        integer(INT32) :: column_i, segment_i, synapse_i, n_active_synapses
        ...
        do column_i = 1, N_COLUMNS
            ...
            do segment_i = 1, columns(column_i) % distal_dendrite_segment_size
                do synapse_i = 1, N_DD_SYNAPSES
                    if (columns(column_i) % distal_dendrite_synapse_permanence(synapse_i, segment_i) >= permanence_threshold) then
                        global_cell_id = columns(column_i) % distal_dendrite_synapse_origin(synapse_i, segment_i)
                        if (get_element_64(active_cells_all(global_cell_id % column), global_cell_id % cell)) then
            ...
            if (something_to_print) then
                write(*, '("column"(1X,I0)": activity of"(1X,I0)" segments:")', advance='no'), column_i, columns(column_i) % distal_dendrite_segment_size
                do segment_i = 1, columns(column_i) % distal_dendrite_segment_size
                    n_active_synapses = 0
                    do synapse_i = 1, N_DD_SYNAPSES
                        if (columns(column_i) % distal_dendrite_synapse_permanence(synapse_i, segment_i) >= permanence_threshold) then
