...
!========================================================================
    ...
    !    of outputs.
        ...
        integer(INT32) :: overlap_local             (N_COLUMNS)
        real(REAL32)   :: boosted_overlap_local     (N_COLUMNS)
        real(REAL32)   :: r
...
#       endif
        ...
        do column_i = 1, N_COLUMNS
            call calc_overlap(                                          &
                ...
                overlap             = overlap_local(column_i))
            
            r = rand_float(0.1)
            if (learn) then 
                boosted_overlap_local(column_i) = overlap_local(column_i) + r
...
#       if _DEBUG
            if (.FALSE.) then
                write(*, '("INFO:SP:compute_sp: overlap:")', advance='no')
                call print_int_array(overlap_local, N_COLUMNS)
            end if
            if (.FALSE.) then
                write(*, '("INFO:SP:compute_sp: boosted_overlap:")', advance='no')
                call print_float_array(boosted_overlap_local, N_COLUMNS)
            end if
#       endif
        ...
        do column_i = 1, N_COLUMNS
            call inhibition(                                            &
                column              = column_i,                         &
                learn               = learn,                            &
                boosted_overlap     = boosted_overlap_local,            &
                sensor_activity     = sensor_activity,                  &
                ! out
            ...
            if (.FALSE.) then
                if (learn) then
                    call update_duty_cycles(column_i, overlap_local(column_i), active_columns(column_i))
...
!$OMP END SIMD
            ...
            block
                ...
                !almost 50% of all time is spend in the next line. Which makes sense.
