...
!========================================================================
    subroutine print_active_cells(bitset)
        ...
        write(*, '(" [n_columns ="(1X,I0)", n_cells ="(1X,I0)"]:")', advance='yes'), n_active_columns, n_active_cells
        if (n_active_columns > 0) then
            do column_i = 1, N_COLUMNS
                if (active_column(column_i)) then
                    write(*, '(i5)', advance='no'), column_i-1
                end if
            end do
            write(*, '')
...
!========================================================================
    subroutine print_sensor_activity(sensor_activity, dimension_1, dimension_2)
        ...
        write(*, '("number of active sensors:"(1X,I0))'), sum
    ...
    subroutine print_segment_activity(                                  &
        ...
        do column_i = 1, N_COLUMNS
            ...
            if (something_to_print) then
                write(*, '("column"(1X,I0)": activity of"(1X,I0)" segments:")', advance='no'), column_i-1, columns(column_i) % dd_segment_count
                do segment_i = 1, columns(column_i) % dd_segment_count
                    n_active_synapses = 0
                    do synapse_i = 1, N_DD_SYNAPSES
                        if (columns(column_i) % dd_synapse_permanence(synapse_i, segment_i) >= permanence_threshold) then
...
!========================================================================
    subroutine print_dd_synapses(column_i)
        integer(INT32), intent(in) :: column_i
        ! local variables
        integer(INT32) :: segment_i, synapse_i, n_segments
        
        n_segments = columns(column_i) % dd_segment_count
        do segment_i = 1, n_segments
            
            write(*, '("Column"(1X,I4)": segment"(1X,I0)"/"(1X,I0)": permanence: ")', advance='no'), column_i-1, segment_i-1, n_segments
            do synapse_i = 1, N_DD_SYNAPSES
                if (columns(column_i) % dd_synapse_permanence(synapse_i, segment_i) >= 0) then
                    write(*, '(xf5.3)', advance='no') columns(column_i) % dd_synapse_permanence(synapse_i, segment_i)
                endif
            end do
            write(*, '')

            if (.TRUE.) then
                write(*, '("Column"(1X,I4)": segment"(1X,I0)"/"(1X,I0)": cell org  : ")', advance='no'), column_i-1, segment_i-1, n_segments
                do synapse_i = 1, N_DD_SYNAPSES
                    if (columns(column_i) % dd_synapse_permanence(synapse_i, segment_i) >= 0) then
                        write(*, '(1X,I5)', advance='no') columns(column_i) % dd_synapse_origin(synapse_i, segment_i)-1
                    endif
                end do
                write(*, '')
            endif
        end do
    end subroutine print_dd_synapses
!========================================================================
    subroutine print_dd_synapses_all()
        ! local variables
        integer(INT32) :: column_i
        
        do column_i = 1, N_COLUMNS
            if (columns(column_i) % dd_segment_count > 0) then
                call print_dd_synapses(column_i)
            end if
        end do
    end subroutine print_dd_synapses_all
!========================================================================
    subroutine print_pd_synapses(column_i)
        integer(INT32), intent(in) :: column_i
        ! local variables
        integer(INT32) :: segment_i, synapse_i, n_segments
        
        write(*, '("Column"(1X,I4)": permanence: ")', advance='no'), column_i-1
        do synapse_i = 1, N_PD_SYNAPSES
            if (columns(column_i) % pd_synapse_permanence(synapse_i) >= 0) then
                write(*, '(xf5.3)', advance='no') columns(column_i) % pd_synapse_permanence(synapse_i)
            endif
        end do
        write(*, '')

        if (.TRUE.) then
            write(*, '("Column"(1X,I4)": cell org  : ")', advance='no'), column_i-1
            do synapse_i = 1, N_PD_SYNAPSES
                if (columns(column_i) % pd_synapse_permanence(synapse_i) >= 0) then
                    write(*, '(1X,I5)', advance='no') columns(column_i) % pd_synapse_origin(synapse_i)-1
                endif
            end do
            write(*, '')
        endif
    end subroutine print_pd_synapses
!========================================================================
    subroutine print_pd_synapses_all()
        ! local variables
        integer(INT32) :: column_i
        
        do column_i = 1, N_COLUMNS
            call print_pd_synapses(column_i)
        end do
    end subroutine print_pd_synapses_all
!========================================================================
    subroutine print_int_array(d, array_size)
        integer(INT32), intent(in) :: d        (:)
        integer(INT32), intent(in) :: array_size
        ! local variables
        integer(INT32) :: i
        do i = 1, array_size
            write(*, '(1X,I0)', advance='no'), d(i)
        end do
        write(*, '')
    end subroutine print_int_array
!========================================================================
    subroutine print_float_array(d, array_size)
        real(REAL32), intent(in) :: d        (:)
        integer(INT32), intent(in) :: array_size
        ! local variables
        integer(INT32) :: i
        do i = 1, array_size
            write(*, '(xf7.4)', advance='no'), d(i)
        end do
        write(*, '')
    end subroutine print_float_array
!========================================================================
end module htm_v2_print