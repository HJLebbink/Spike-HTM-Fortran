...
!========================================================================
module htm_v2_network
    use, intrinsic :: ISO_FORTRAN_ENV, only: INT8, INT16, INT32, INT64, REAL32, REAL64
    use tools
    use htm_v2_constants
    use htm_v2_tools
    use htm_v2_encoder, only: encode_pass_through, fetch_sensor_activity, print_sensor_activity2, print_inferred_sensor_activity, encoder_cleanup
    use htm_v2_sp,      only: compute_sp
    use htm_v2_tm,      only: compute_tm
    use htm_v2_print
    
    implicit none

        logical(1), allocatable      :: active_cells_all             (:)
        logical(1), allocatable      :: prev_active_cells_all        (:)
        logical(1), allocatable      :: winner_cells_all             (:)
        logical(1), allocatable      :: prev_winner_cells_all        (:)
        logical(1), allocatable      :: predictive_cells_all         (:)
        logical(1), allocatable      :: prev_predictive_cells_all    (:)
...
!========================================================================
    subroutine run(n_time_steps, progress, n_times, param)
        ...
        do times = 1, n_times
            ...
            ! reset the network
            call clear_all(active_cells_all)
            call clear_all(winner_cells_all)
            call clear_all(predictive_cells_all)
            call init(param)
            total_mismatch = 0
            ...
            do t = 1, n_time_steps
...
#if _DEBUG
                    if (.FALSE.) then
                        write(*, '("INFO:network:run: active columns at t ="(1X,I0)":")', advance='yes'), t
                        call print_active_columns(active_columns, NINT(SQRT(REAL(N_COLUMNS))), NINT(SQRT(REAL(N_COLUMNS))))
                    end if
                    if (.FALSE.) then
                        write(*, '("INFO:network:run: active cells at t ="(1X,I0)": ")', advance='yes'), t
                        call print_active_cells(active_cells_all)
                    end if
                    if (.FALSE.) then
                        write(*, '("INFO:network:run: predictive cells at t ="(1X,I0)": ")', advance='yes'), t
                        call print_active_cells(predictive_cells_all)
                    end if
                    if (.FALSE.) then
                        write(*, '("INFO:network:run: dd_synapes at t ="(1X,I0)": ")', advance='yes'), t
                        call print_dd_synapses_all()
                    end if
                    if (.FALSE.) then
                        write(*, '("INFO:network:run: pd_synapes at t ="(1X,I0)": ")', advance='yes'), t
                        call print_pd_synapses_all()
                    end if
#endif
                ...
                if (.TRUE.) then
                    ...
                    if (t-1 == 0) then
                        write(*, '("INFO:network:run: total mismatch: ")', advance='no')
                    end if
                    if ((MOD(t-1, 10) == 0).AND.(t > 1)) then
                        write(*, '(1X,I3)', advance='no'), total_mismatch
                        total_mismatch = 0
                    end if
                end if
                if (.FALSE.) then
                    total_mismatch = calc_mismatch(t, predictive_cells_all)
                    write(*, '("INFO:network:run: mismatch at t ="(1X,I0)":"(1X,I0))', advance='yes'), t-1, total_mismatch
                end if
...
!========================================================================
    function calc_mismatch(t, predictive_cells) result(r)
        ...
        logical(1),     intent(in) :: predictive_cells      (N_CELLS)
        integer(INT32) :: r
        ! local variables
...
!========================================================================
    subroutine print_progress(t, active_cells, active_columns, predictive_cells)
        integer(INT32), intent(in) :: t
        logical(1),     intent(in) :: active_cells          (N_CELLS)
        ...
        if (MODULO(t, progress_frequency) == 0) then
            ...
            if (.FALSE.) then ! print active cells
                print '("=====")'
            ...
            if (.TRUE.) then ! print predicted sensor activity
                print '("=====")'
                print '("at t ="(1X,I0)": predicted sensor activity at (future) t ="(1X,I0)":")', t-1, t
                print '("       predicted |          correct |         mismatch")'
                call get_predicted_sensor_activity(predictive_cells, inferred_sensor_activity)
                sensor_activity1 = inferred_sensor_activity>1
                call fetch_sensor_activity(t+1, sensor_activity2)
...
!========================================================================
    subroutine allocate_memory(param)
        type(param_t), intent(in) :: param
        ! local variables
        ...
        allocate(active_cells_all(param % n_cells), STAT = allocateStatus)
        if (allocateStatus /= 0) stop "*** Not enough memory ***"
        allocate(prev_active_cells_all(param % n_cells), STAT = allocateStatus)
        if (allocateStatus /= 0) stop "*** Not enough memory ***"
        allocate(winner_cells_all(param % n_cells), STAT = allocateStatus)
        if (allocateStatus /= 0) stop "*** Not enough memory ***"

        allocate(prev_winner_cells_all(param % n_cells), STAT = allocateStatus)
        if (allocateStatus /= 0) stop "*** Not enough memory ***"
        allocate(predictive_cells_all(param % n_cells), STAT = allocateStatus)
        if (allocateStatus /= 0) stop "*** Not enough memory ***"
        allocate(prev_predictive_cells_all(param % n_cells), STAT = allocateStatus)
        if (allocateStatus /= 0) stop "*** Not enough memory ***"
...
!========================================================================
    subroutine init(param)
        ...
        do column_i = 1,param % n_columns
            columns(column_i) % dd_segment_count = 0

            do synapse_i = 1, N_PD_SYNAPSES
                columns(column_i) % pd_synapse_permanence(synapse_i) = SP_PD_PERMANENCE_INIT
                columns(column_i) % pd_synapse_origin(synapse_i) = rand_int32(0, N_SENSORS-1) + 1

#if _DEBUG
                    if (.FALSE.) print '("INFO:constants::init: column "(1X,I0)"; synapse "(1X,I0)"; pd_synapse_origin "(1X,I0)"; current_random_number"(1X,I0))', column_i, synapse_i, columns(column_i) % pd_synapse_origin(synapse_i), current_random_number
#endif

            end do
            
            if (.FALSE.) then ! sort the proximal dendrites
                call sort(columns(column_i) % pd_synapse_origin)
            end if
        end do
    end subroutine init
!========================================================================
    subroutine get_inferred_sensor_activity(active_columns, inferred_sensor_activity)
        ...
        do column_i = 1,N_COLUMNS
            if (active_columns(column_i)) then
                do synapse_i = 1, N_PD_SYNAPSES
                    if (columns(column_i) % pd_synapse_permanence(synapse_i) >= SP_PD_PERMANENCE_THRESHOLD) then
                        sensor_idx = columns(column_i) % pd_synapse_origin(synapse_i)
...
!========================================================================
    subroutine get_predicted_sensor_activity(predicted_cells, predicted_sensor_activity)
        logical(1),     intent(in)  :: predicted_cells              (N_CELLS)
        integer(INT32), intent(out) :: predicted_sensor_activity    (N_SENSORS)
        ! local variables
        integer(INT32) :: global_cell_id, column, synapse_i, sensor_idx
        integer(INT8)  :: cell

        predicted_sensor_activity = 0
        
        do global_cell_id = 1,N_CELLS
            if (predicted_cells(global_cell_id)) then
                call global_2_local_cell(global_cell_id, cell, column)
                do synapse_i = 1, N_PD_SYNAPSES
                    if (columns(column) % pd_synapse_permanence(synapse_i) >= SP_PD_PERMANENCE_THRESHOLD) then
...
!========================================================================
    ...
    end function estimate_memory_usage_byte
!========================================================================
end module htm_v2_network
